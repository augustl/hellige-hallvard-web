...
export default async function RootLayout({children}: {children: React.ReactNode}) {
    const wpPagesData: any[] = await (
        await fetch(
            `${process.env.NEXT_PUBLIC_WORDPRESS_API_URL}/pages?context=embed&per_page=100&orderby=menu_order&order=asc&exclude=${process.env.NEXT_PUBLIC_HOME_PAGE_ID}&parent=0`,
            {next: {tags: ["wp-pages"]}}
        )
    ).json()
    ...
    return (
        <html lang="no">
            <body
                className={`${inter.className} flex min-h-screen flex-col items-center text-base`}
            >
                <nav
                    className="flex flex-col gap-5 md:gap-10 items-center mt-10 mb-10 md:mb-20 px-5 print:hidden"
                    role="navigation"
                >
                    ...
                    <Link
                        href="/"
                        title="Hellige Halvard ortodokse menighet - forsiden"
                        className="font-bold text-xl md:text-3xl font-serif text-center"
                    >
