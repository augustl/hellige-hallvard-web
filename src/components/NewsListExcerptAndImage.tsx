...
const DownsizedWordpressImage: React.FC<{imageId: number}> = async ({imageId}) => {
    const wpMediaDataRes = await fetch(`${process.env.NEXT_PUBLIC_WORDPRESS_API_URL}/media/${imageId}`)
...
export default async function NewsListExcerptAndImage({currentPage}: {currentPage: number}) {
    const perPage = 10
    const wpPostsDataRes = await fetch(`${process.env.NEXT_PUBLIC_WORDPRESS_API_URL}/posts?context=view&per_page=${perPage}&page=${currentPage}`, {next: {tags: ["wp-posts"]}})
