...
async function WpPostList() {
    const res = await fetch(
        `${process.env.NEXT_PUBLIC_WORDPRESS_API_URL}/posts?context=embed&per_page=6`,
        {next: {tags: ["wp-home-page", "wp-posts"]}}
    )
            ...
            {wpPostsData.map(it => {
                ...
                return (
                    <React.Fragment key={it.id}>
                        <div>
                            {d}.{m}
                            {y === now.getFullYear().toString() ? `` : `.${y}`}
                        </div>
                        <div>
                            <Link
                                href={`/nyheter/${y}/${m}/${it.slug}`}
                                className="font-bold hyphens-auto w-full"
...
async function WpHomePage() {
    const res = await fetch(
        `${process.env.NEXT_PUBLIC_WORDPRESS_API_URL}/pages?context=view&include=${process.env.NEXT_PUBLIC_HOME_PAGE_ID}`,
        {next: {tags: ["wp-home-page"]}}
    )
    ...
    return (
        <div
            className="hh-typography hh-content-blocks"
...
export default async function HomePageForDate({date}: {date: moment.Moment}) {
    ...
    const thisYearsPaschaCycleStart = DateTime.fromJSDate(
        new Date(
            date.year(),
            thisYearsPaschaCycleStartData[0] - 1,
            thisYearsPaschaCycleStartData[1]
        )
    )
    const currentDaysSinceStartOfPaschalCycle = DateTime.fromJSDate(date.toDate()).diff(
        thisYearsPaschaCycleStart,
        "days"
    ).days

    const isInGreatLent =
        currentDaysSinceStartOfPaschalCycle >= 14 && currentDaysSinceStartOfPaschalCycle <= 84
                ...
                <div className="alignwide">
                        ...
                        {isInGreatLent && (
                            <div className="border-4 border-yellow-600 border-double inline-block relative">
                                <h2 className="px-4 py-2 bg-yellow-100 dark:bg-yellow-900 bg-opacity-40 text-xl font-bold font-serif">
                                    <span className="whitespace-nowrap">𐡷 Hl.</span> Efraim Syrerens{" "}
                                    <span className="whitespace-nowrap">bønn 𐡸</span>
                                </h2>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-10">
                        <div className="md:order-2">
                            ...
                            <p className="hh-body-typography mt-4">
                                <Link href="/gudstjenester">Hele kirkekalenderen</Link>
                            </p>
                        </div>

                        <div className="md:order-3">
                            <h2 className="mb-4 text-2xl font-bold font-serif">
                                Neste helgen/festdag
                            </h2>
                            <DagensHoytid date={date} />
                            <p className="hh-body-typography mt-4">
                                <Link href="/helgen-og-festkalender">
                                    Hele helgen- og festkalenderen
                                </Link>
                            </p>
                        </div>
                        ...
                        <div className="md:order-1">
                            ...
                            <p className="hh-body-typography">
                                <Link href="/nyheter/side/1">Alle nyheter</Link>
                            </p>
                        </div>
                    </div>
                </div>
