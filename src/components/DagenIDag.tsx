    ...
    return (
        <div>
            ...
            <div className="flex flex-col gap-4 dark:text-gray-300 text-gray-700">
                ...
                <div>
                    <div className="border-4 border-slate-300 dark:border-slate-600 border-double inline-block relative max-w-md">
                        <div className="px-4 py-2 bg-slate-200 dark:bg-slate-600 bg-opacity-40 font-bold flex flex-row gap-4">
                            ...
                            <Link
                                href={`/dagenstekster/${y}/${m}/${d}`}
                                className="flex flex-row gap-1"
                                title="Kalender med dagens tekster"
                            >
...
async function DagensHendelserList({now}: {now: moment.Moment}) {
    ...
    const res = await fetch(
        `https://www.googleapis.com/calendar/v3/calendars/${encodeURIComponent(
            ...
            .NEXT_PUBLIC_GCAL_BACKEND_API_KEY!}&maxResults=4&timeMin=${now.toISOString()}&timeMax=${tomorrow.toISOString()}&timeZone=Europe/Oslo&orderBy=startTime&singleEvents=true`,
        {next: {revalidate: 60}}
    )
    ...
    return (
        <div className="flex flex-col gap-4">
            {upcomingEvents.map(event => {
                ...
                return (
                    <div key={event.id} className="hh-body-typography text-lg">
                        {event.isFullDayEvent ? (
                            <></>
                        ) : (
                            <div>
                                <span className="font-bold">
                                    {(!event.url && event.location) || "Kirken"}, {hour}:{minute}
                                </span>
                            </div>
                        )}{" "}
...
async function DagensHoytidList({now}: {now: moment.Moment}) {
    ...
    const res = await fetch(
        `https://www.googleapis.com/calendar/v3/calendars/${encodeURIComponent(
            process.env.NEXT_PUBLIC_HOYTIDER_GCAL_ID!
        )}/events?key=${process.env
            .NEXT_PUBLIC_GCAL_BACKEND_API_KEY!}&maxResults=5&timeMin=${now.toISOString()}&timeMax=${tomorrow.toISOString()}&timeZone=Europe/Oslo&orderBy=startTime&singleEvents=true`,
        {next: {revalidate: 60}}
    )
    const upcomingEvents = await getGoogleCalendarUpcomingEvents(await res.json())
    ...
    return (
        <div className="mb-2">
            {upcomingEvents.map(event => {
                return (
                    <div key={event.id} className="text-xl font-serif">
                        {event.summary}
                    </div>
                )
            })}
