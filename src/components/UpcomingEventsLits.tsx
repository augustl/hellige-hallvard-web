...
export default function UpcomingEventsList() {
    ...
    useEffect(() => {
        // List all events for today and later
        const calendarCutoffTime = new Date()
        calendarCutoffTime.setHours(0, 0, 0, 0)

        fetch(`https://www.googleapis.com/calendar/v3/calendars/${encodeURIComponent(process.env.NEXT_PUBLIC_GCAL_ID!)}/events?key=${process.env.NEXT_PUBLIC_GCAL_API_KEY!}&maxResults=3&timeMin=${calendarCutoffTime.toISOString()}&timeZone=Europe/Oslo&orderBy=startTime&singleEvents=true`)
            .then((res) => res.json())
